...
- 导出包 v2 起身份表随包恢复（不含会话）；见 [库存合同 V6-3](inventory-v6.md) 第 4 节。会话仍属运行期，恢复后由配置主体引导重新签发。**【已实现】**
- 角色目前只影响管理接口与既有变更授权（observer 只读）；对象级细粒度权限见 [权限合同 V6-2](permits-v6.md)（已实现并验证）。
- 无速率独立的暴力破解防护：令牌为 256 位随机、仅存哈希，认证失败即断开并计入五秒认证超时与每秒消息限额；专项目枚举防护未实现。**【已实现】**
- HTTPS 资产端点已改为对每次请求读取 RegionStore 持久会话：新签发账户无需重启网关即可下载获准资产；会话撤销、过期或账户禁用后下一次请求即返回 401。资产目录仍按账户 ID 判断授权，文件仍验证 SHA-256；存储不可用返回 503。**【已验证，2026-09-26】**
...
后续资产会话修复：`services/Test-AssetSessions.cjs` 13/13（动态签发、授权、收回、会话撤销、禁用、过期、未知哈希与内容摘要），使用独立 RegionStore 数据库和 HTTPS 网关验收；扩展的 `services/Test-Identity.cjs` 43/43，在真实 Godot 权威服务中覆盖动态账户、授权目录同步及 `world_model` 命令来源的持久回执。
