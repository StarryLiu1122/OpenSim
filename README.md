# Region Lab：可探索、可建造的共享三维街区

这个分支提供基于 Godot 的**单区域共享世界原型**。当前可打开赫尔辛基 **250 × 250 米**航拍建模街区：走路、快跑、飞行、查看地点，预览并放置物体，调整自己的物体，以及保存和复用个人模型。场景由 64 块带照片纹理的网格组成，放在 512 × 512 米的区域内。平台现阶段优先改善探索、建造与多人使用体验；世界模型与智能体实验是独立的可选演示。

网络客户端把「探索」「建造」「我的素材」放在主要入口。建造页提供放置预览、移动、旋转、缩放、组合，以及地形和环境控件；模型上传成功后会进入个人素材库，重新连接后仍可使用。详细操作见[共享世界客户端指南](prototype/docs/network-interface.md)，后续任务见[体验实施路线](docs/plans/product-experience-roadmap.md)。

![新版共享工作台中的赫尔辛基 250 米街区总览](prototype/docs/images/helsinki-current-workspace.png)
...
地点菜单还可以“在此建造方块”“在此放置当前模型”和“聚焦此处”。在「建造」页选择「预览方块位置」，或在「我的素材」选择模型预览：鼠标移动轮廓，左键确认，**R** 旋转，**Esc** 取消。选中自己的独立物体后可按 **G** 移动，或使用旋转、缩放控件；名称与坐标须点击「保存名称和位置」提交。Ctrl+单击可多选后组合。服务端回执到达前，修改不视为保存成功。只读账户可以查看和漫游，不能建造。

导入自己的静态模型：在「我的素材」页选择 **GLB / glTF / OBJ**，填写名称、真实许可与来源，点击「提交模型资产」；收到成功回执后，会自动存入个人素材库，可预览位置后放进场景。桌面 OBJ 可使用同目录的 MTL 和 PNG/JPEG。单件转换后的 GLB 最多 **2 MiB**、**20,000 个三角面**，世界最多 **96 个导入网格**。JSON 世界存档的元数据限 **8 MiB**，模型字节另存在同名 `.assets` 目录；复制或备份时须携带两者。具体支持格式与拒绝原因见[模型导入说明](prototype/docs/quality-import-and-navigation.md)。

## 当前范围与更多资料

赫尔辛基场景来自 **2017 年航拍重建**，有原始照片纹理，但近距离表面和建筑室内还不够精细；当前展示的是一个静态街区，不是整座城市。来源、许可和转换过程见[场景说明](prototype/docs/helsinki-textured-city-pilot.md)。场景容量上限已放宽，但服务进程仍会在内存中保留世界资产；自动绕障、小地图、公开互动、共享撤销、分块加载和 LOD 尚未完成。模型预测的受控标记回注与审阅已在单场景验证；实时平台回调和科学推演未完成，见[版本进度](prototype/docs/v6-v7-progress.md)。
