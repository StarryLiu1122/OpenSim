...
if ($nativeReport.failed -ne 0 -or $nativeReport.passed -lt 46) { throw 'Native tests did not complete.' }
...
if ($Visual) {
    ...
    if ($visualReport.failed -ne 0 -or $visualReport.passed -lt 18) { throw 'UI verification did not complete.' }
    foreach ($name in @('overview.png', 'edited.png')) {
