...
if (!(Test-Path -LiteralPath (Join-Path $instance 'private-config.json'))) {
...
if (Test-Path -LiteralPath $processFile) {
    ...
    $running = [bool](Get-Process -Id $processes.server_pid -ErrorAction SilentlyContinue) -and [bool](Get-Process -Id $processes.gateway_pid -ErrorAction SilentlyContinue)
}
