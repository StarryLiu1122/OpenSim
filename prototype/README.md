...
V5 网络客户端现采用独立工作区：顶部提供连接状态与醒目的漫游入口，右侧按场景、属性、连接、高级和资产分区，底部显示操作结果。支持 Tab 进入漫游、场景点选、对象搜索与定位、编辑草稿保护、删除确认，以及通过表单上传模型并放入场景。右键地点菜单、镜头快捷键和导入选择见[清晰建筑资源、导入与漫游](docs/quality-import-and-navigation.md)。重新打开桌面网络客户端即可加载新界面；浏览器部署需要重新导出 Web 文件。
