...
网络客户端现采用独立工作区：顶部提供连接状态与醒目的漫游入口，右侧按场景、属性、连接、高级、资产和推演审阅分区，底部显示操作结果。支持 Tab 进入漫游、场景点选、对象搜索与定位、编辑草稿保护、删除确认、模型上传，以及对世界模型预测点做本地专家审阅。右键地点菜单、镜头快捷键和导入选择见[清晰建筑资源、导入与漫游](docs/quality-import-and-navigation.md)；预测点操作见[世界模型实验](docs/world-model-experiment.md)。重新打开桌面网络客户端即可加载新界面；浏览器部署需要重新导出 Web 文件。
...
支持范围：静态 GLB 2.0，桌面端可把 glTF 或 OBJ/MTL/PNG/JPEG 转成 GLB；打包结果单文件不超过 2 MiB、不超过 20,000 三角形（包括碰撞代理）、每轴尺寸 0.2–32 米、不透明材质、内嵌 PNG/JPEG 基础色贴图。动画、压缩扩展等会被拒绝，完整限制及导出要求见[组合与资产规范](docs/groups-and-assets.md)。快照整体上限仍为 8 MiB。
