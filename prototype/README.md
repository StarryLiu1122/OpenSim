...
版本：0.5.1。本文主要描述保留的 Windows x64 离线编辑器。**共享区域、桌面和 Web 客户端请从 [网络运行指南](docs/network-quickstart.md) 开始。** 世界格式仍为 3，本地命令版本仍为 1，现代网络协议为 FP 0.3；网络 RegionStore 数据库 schema 为 7。
...
网络客户端采用独立工作区：顶部提供连接状态与漫游、建造、我的素材入口；右侧还有探索、连接、高级和推演审阅页面，底部显示提交结果。支持 Tab 漫游、点选与地点菜单、放置预览、移动/旋转/缩放、组合、地形和环境控件、个人素材库以及模型上传。操作见[共享世界客户端指南](docs/network-interface.md)；预测点操作见[世界模型实验](docs/world-model-experiment.md)。重新打开桌面网络客户端即可加载新界面；浏览器部署需要重新导出 Web 文件。

Region Lab 是基于 Godot 的单区域原型，支持对象组合、静态 GLB、六类内置对象、程序材质、地形、环境、门灯交互和角色漫游。新世界提供庭院、湖岸及可进入的展馆。区域可为 256×256 或 512×512 米；离线模式为本机单用户，网络模式由独立权威服务和多个只读投影客户端组成。
...
1. 打开右侧“资产”页，选择 GLB、glTF 或 OBJ 文件，或输入文件路径。
...
5. 保存后模型内容按 SHA-256 外置。JSON 世界文件与同名 `.assets` 目录必须一起搬移或备份；SQLite 数据库实例使用 `objects/` 内容目录和导出包。没有实例引用的资产可在资产页移除，移除可撤销。

支持范围：静态 GLB 2.0，桌面端可把 glTF 或 OBJ/MTL/PNG/JPEG 转成 GLB；打包结果单文件不超过 2 MiB、不超过 20,000 三角形（包括碰撞代理）、每轴尺寸 0.2–32 米、不透明材质、内嵌 PNG/JPEG 基础色贴图。动画、压缩扩展等会被拒绝，完整限制及导出要求见[组合与资产规范](docs/groups-and-assets.md)。一个世界最多 96 个导入网格；JSON 快照元数据上限为 8 MiB。
