...
先查看 `$report` 的 `ok: true` 与 `world_revision`，再启动服务。若写入结果不确定，按 [RegionStore 说明](../../services/README.md)用**同一请求 ID**查询或原样重试；不要用新的 ID 猜测重发。客户端初始镜头朝向区域出生点。当前世界有单区域 256 × 256 米、最多 500 个对象、最多 16 个导入网格资产等限制；需要完整校园尺度，要先完成多区域管理、流式加载和层级细节（LOD）。
