...
| glb | 运行时世界记录中的 GLB Base64；新版 JSON 快照和 SQLite 行只保留 SHA-256 引用，字节另存于内容目录 |

目录前六项仍须为固定内置资产，之后最多 96 个网格资产。不同内容即使名称相同也具有独立记录。资产字段不可通过对象编辑修改。单件限制与元数据快照限制分别执行；服务进程当前仍在内存中保留全部模型字节，96 是目录上限，不是已验证的低内存容量。
...
| 大小 | 单文件 ≤2 MiB；JSON 块 ≤256 KiB；世界元数据快照序列化后 ≤8 MiB，模型字节另存 |
...
保存时记录直接选择或从 glTF 打包得到的 GLB。新版 JSON 快照封装为版本 2：世界元数据保存在主文件，模型字节按完整 SHA-256 存于同名 `.assets` 目录。复制、备份或迁移时要同时携带主文件和此目录；缺失或摘要不符的资源会阻止读取。版本 1 的内嵌快照仍可读取，首次显式保存后转为版本 2。SQLite 模式使用数据库实例的 `objects/` 内容目录，导出包携带模型内容。读取时仍验证完整摘要、结构和包围尺寸，不自动下载缺失资源。历史和服务器世界仍在内存中保存模型字节；分块加载、LRU 与 LOD 尚未完成。
