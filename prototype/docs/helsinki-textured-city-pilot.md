...
这是 2017 年航拍网格的一个静态片段，不代表当前建筑现状。片区以外仍为原型自带的程序地形；模型边缘可能出现航拍重建缝隙，建筑室内没有单独建模。导入上限现为 96 个资产；此 16 块演示保留为轻量入口。系统尚不能连续加载整座城市或通用 3D Tiles。
...
复现：`python prototype/tools/Convert-HelsinkiMesh.py <672495a3源目录> <全新输出目录> --grid-x=0 --grid-y=0 --count=8`。再用 `build_city_pilot.gd` 的 `--manifest`、`--world-file` 和 `--report` 参数构建独立世界；也可直接使用仓库内的 64 块转换结果。原始版本 1 存档约 6.93 MB；新版存档把二进制移到同名 `.assets` 目录，64 块场景还可加入最多 32 件新模型。隔离副本已验证加入 10 件不同模型后冷启动与备份恢复；分块加载仍未实现。客户端已有独立网络实例验证 64 块模型、64 张纹理和可行走出生点，不能据此宣称低端设备帧率或近距离摄影测量质量达标。
