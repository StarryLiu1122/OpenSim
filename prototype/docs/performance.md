...
测量日期：2026-09-16，应用 0.5.1。固定 44 对象场景，单线程 Godot 4.5.1 / Jolt / WebGL2，两个内核各五次独立浏览器进程和临时配置。由本机 HTTPS 网关对静态文件及资产响应体合计施加 50 Mbps 节流。从导航开始，计至认证、必要资产验证、碰撞投影及首个可操作渲染状态完成。

| 浏览器 | 冷启动范围 / 秒 | 冷启动中位数 / 秒 | 热启动 / 秒 | 五次均 ≤10 秒 |
| --- | --- | --- | --- | --- |
| Chromium 151.0.7922.34 | 7.615–8.869 | 7.639 | 3.619 | 通过 |
| Firefox 153.0 | 10.236–10.678 | 10.377 | 8.558 | **未通过** |

硬件为 Core Ultra 7 255HX、约 16 GB 内存、Windows 11，浏览器报告 Intel 图形 / ANGLE D3D11。浏览器上下文配置 1440×960 CSS 视口、deviceScaleFactor=1；系统显示缩放可能影响截图物理尺寸。Web 使用顶点光照、Lambert、关闭动态阴影和 MSAA，桌面保留原设置。

上述为同机短时启动基线，没有模拟 WAN 延迟、抖动或丢包，未清操作系统和驱动缓存，不能视为公网体验或容量保证。冷启动包含自动填写令牌的时间；热启动单独统计，不代替冷样本。Firefox 超预算，因此 V5.1-06 保持未通过。

逐次计时、资源与启动阶段见 [原始 Web 报告](../../docs/comparisons/evidence/v5-20260916/web-report.json)；完整方法、硬件和优化记录见 [V5 验收记录](../../docs/comparisons/v5-completion.md#3-web-冷启动与性能结论)。下文保留 V3.1 的独立离线基线，两个版本的场景和计时起点不同，不作直接性能增减比较。
