...
新版工作区完成 39 项网络界面检查，其中 7 项为截图生成；实际截图另行检查。原生 191 项、原有离线界面 66 项、网络数据合同 16 项和双桌面网络回归 64 项通过；跨进程恢复和三组批处理通过。测试均使用独立目录和端口。操作、适用范围与复现命令见 [界面说明](network-interface.md)，报告及截图见 [界面证据](../../docs/comparisons/evidence/v5-ui-20260920/README.md)。

Chromium / Firefox 最终完整复测 76 / 76 通过，各五个冷启动样本均小于 10 秒。前序出现的 Firefox 指针锁时序问题已修正；另有一次启动超时在显式激活测试页面后的复测中未再出现，底层原因未完全定位，失败记录仍保留。该专项未重新制作和验收完整离线安装包；历史基线和本轮复测应分别理解。

## 2026-09-16：V5 功能基线（历史）
