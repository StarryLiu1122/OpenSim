...
func build() -> void:
	...
	button(assets, "选择新的 GLB…" if OS.has_feature("web") else "选择新的 GLB / glTF / OBJ…", app._pick_file)
	upload_note = note(assets, "先在场景页选择 GLB 文件。" if OS.has_feature("web") else "先在场景页选择 GLB、glTF 或 OBJ 文件。")
