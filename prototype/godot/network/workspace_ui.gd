...
func build() -> void:
	...
	import_button = button(scene, "导入 GLB 模型…", app._pick_file)
	note(scene, "GLB：静态模型，最大 2 MiB。选择后填写来源与许可，再提交资产。")
	...
	button(assets, "选择新的 GLB…", app._pick_file)
	upload_note = note(assets, "先在场景页选择 GLB 文件。")
