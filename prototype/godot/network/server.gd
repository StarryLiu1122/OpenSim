...
func _receive(client: Dictionary, packet: Dictionary) -> void:
	...
	match packet.type:
		...
		"input":
			...
			var base := ["fp_version", "type", "sequence", "axis", "yaw", "jump"]
			if not Schema.exact_keys(packet, base) and not Schema.exact_keys(packet, base + ["sprint"]) and not Schema.exact_keys(packet, base + ["fly", "lift"]) and not Schema.exact_keys(packet, base + ["sprint", "fly", "lift"]): return
			...
			if packet.has("fly") and (not packet.fly is bool or not Schema.number(packet.lift, -1, 1)): return
			...
			client.avatar.flying = packet.get("fly", false); client.avatar.lift = float(packet.get("lift", 0.0)); client.avatar.world_size = service.model.region_size()
		"query_result":
