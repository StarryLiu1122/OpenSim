...
func run(suite: SceneTree) -> void:
	...
	suite.check(service.request("SaveRegion").ok, "group-capable world saves mesh bytes outside the snapshot")
	var stored: Dictionary = JSON.parse_string(FileAccess.get_file_as_string(service.repository.path))
	suite.check(stored.version == 2 and not stored.world_json.contains("\"glb\""), "version 2 snapshot keeps only content references")
	...
	DirAccess.make_dir_recursive_absolute(moved_path + ".assets")
	for name in DirAccess.get_files_at(service.repository.path + ".assets"):
		DirAccess.copy_absolute(service.repository.path + ".assets/" + name, moved_path + ".assets/" + name)
	Assets._cache.clear()
	var reader = Service.new(moved_path)
	suite.check(reader.request("LoadRegion").ok and suite._same_data(reader.model.snapshot().assets, service.model.snapshot().assets) and reader.model.object(item.id).asset_id == asset.id and reader.model.snapshot().groups.size() == 1, "relocated snapshot and content directory restore geometry with empty cache")
	var content_path: String = moved_path + ".assets/" + asset.sha256 + ".glb"
	var held_path: String = content_path + ".held"
	DirAccess.rename_absolute(content_path, held_path)
	var before_missing: Dictionary = reader.model.snapshot()
	suite.check(not reader.request("LoadRegion").ok and suite._same_data(reader.model.snapshot(), before_missing), "missing content rejects snapshot without changing the live world")
	DirAccess.rename_absolute(held_path, content_path)
	suite.check(reader.request("LoadRegion").ok, "restored content makes the snapshot loadable again")
	var good_bytes := FileAccess.get_file_as_bytes(content_path)
	var damaged := FileAccess.open(content_path, FileAccess.WRITE)
	damaged.store_string("broken"); damaged.close()
	suite.check(not reader.request("LoadRegion").ok and suite._same_data(reader.model.snapshot(), before_missing), "corrupt content rejects snapshot without changing the live world")
	var repaired := FileAccess.open(content_path, FileAccess.WRITE)
	repaired.store_buffer(good_bytes); repaired.close()
	suite.check(reader.request("LoadRegion").ok, "restored checksum makes the snapshot loadable again")
	var v1_path: String = suite.output_dir + "/legacy-embedded.json"
	var v1_world_json := JSON.stringify(service.model.snapshot(), "\t", true, true)
	var v1_envelope := {"format": "region-lab.snapshot", "version": 1, "sha256": v1_world_json.sha256_text(), "world_json": v1_world_json}
	var v1_file := FileAccess.open(v1_path, FileAccess.WRITE)
	v1_file.store_string(JSON.stringify(v1_envelope, "\t", true, true) + "\n"); v1_file.close()
	var legacy_reader = Service.new(v1_path)
	suite.check(legacy_reader.request("LoadRegion").ok and suite._same_data(legacy_reader.model.snapshot().assets, service.model.snapshot().assets), "version 1 embedded snapshot still loads")
	suite.check(legacy_reader.request("SaveRegion").ok and JSON.parse_string(FileAccess.get_file_as_string(v1_path)).version == 2, "saving a version 1 snapshot migrates it to references")
